...
export default class Focus extends Component {
    ...
    unDeleteHandler(object, id) {
        axios.get(`/${object}/${id}/undestroy`).then(() => {
            this.props.clearFocus();
            this.props.refreshData();
        });
    }
    ...
    assignCar(carId) {
        if (this.state.optionChoice !== 0) {
            axios
                .put(`/customers/${this.props.object.id}/addCar/${carId}/`)
                .then(() => {
                    this.props.refreshData();
                });
        }
    ...
    removeCar(carId) {
        axios
            .put(`/customers/${this.props.object.id}/removeCar/${carId}/`)
            .then(() => {
                this.props.refreshData();
            });
    }
    ...
    chooseCar() {
        return (
            <div>
                ...
                <button
                    className="focus-form-select-button"
    ...
    displayList(id) {
        ...
        if (
            this.state.objectName.toString() === "editedCars" ||
            this.state.objectName.toString() === "editedZoomed" ||
            this.state.objectName.toString() === "editedAlerts"
        ) {
            return (
                <div className="list-wrapper">
                    ...
                    <div
                        ...
                        onClick={() => {
                            this.setState({
                                ...
                                editedObject: { ...this.state.editedCustomers }
                            });
                        }}
                    >
                        {this.state.editedCustomers.name}{" "}
                        {this.state.editedCustomers.surname}
                    </div>
    ...
    displayActions(id) {
        ...
        if (
            ...
            this.state.objectName.toString() === "editedZoomed" ||
            this.state.objectName.toString() === "editedAlerts" ||
            this.state.objectName.toString() === "editedDeletedCars"
        ) {
            return (
                <div className="list-wrapper">
